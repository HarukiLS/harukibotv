...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:ʜᴀʀᴜᴋɪ ʟs\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=554989256326:+55 49 8925-6326\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '.'
limitawal = 30
...
/******** OWNER NUMBER**********/
const ownerNumber = ["554989256326@s.whatsapp.net","554988819670@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* leia as regras e não seja um ghost`
				...
				teks = `menos um🥳 @${num.split('@')[0]} já vai tarde
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           return false
            ...
            if (bad.includes(messagesC)) {
				...
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('Só não te meto o ban porque tu é ADM👮')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`Vai mandar link na puta que te pariu ${sender.split("@")[0]} vagabundo do krl, sabe pedir não?`)
						setTimeout( () => {
			...
			switch(command) { 
				...
				case 'pornhub':
					...
					var porn = gh.split("&")[0];
					var hub = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}pornhub Haruki & Hub`)
					reply(ind.wait())
                ...
                case 'glitchtext':
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}glitchtext Haruki & LS`)
					reply(ind.wait())
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                if (namaUser.length >= 10) return reply(`Nome maior que meu pau, vsf`)
                if (umurUser > 25) return reply(`sua idade é muito alta, máximo 25 anos`)
                if (umurUser < 12) return reply(`sua idade é muito baixa, mínimo 12 anos`)
                try {
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("|")[1];
					const pref = `Use: \n${prefix}quotemaker texto|marcadágua\n\nEx :\n${prefix}quotemaker Meu ovo|Haruki`
					if (args.length < 1) return reply(pref)
					reply(ind.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=random`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Toma', quoted: mek})
					await limitAdd(sender)
					break
				//fadli 
				case 'pinterest':
				...
				case 'quotes':
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length < 1) return reply(`Entrada Número\nExemplo : ${prefix}infonomor 0812345678`)
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
              	    if (args.length < 1) return reply('onde tá o texto, poha?')
                    teks = `${body.slice(8)}`
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Esse é o número do Haruki, não incomoda ele senão vai tomar block',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				bo = args[0]
...
• Limite: ${limitawal-updated.limit}`
                            ...
                            reply(result)
                                ...
                                reply(`Desculpe, número ${nomerr} não registrado no banco de dados do 🤖ʜᴀʀᴜᴋɪ ʙᴏᴛ🤖!`)
                        }
                break
				case 'premlist':
	            case 'listprem':
            ...
            break
                ...
                reply(`*「 SUCESSO 」*\n\nA transferência de dinheiro foi bem sucedida\nDe : +${sender.split("@")[0]}\nPara : +${tujuan}\nValor da transferência : ${jumblah}\nImposto : ${fee}`)
                break
				case 'dompet':
				if (!isRegistered) return reply(ind.noregis())
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Marque uma sticker, poha!!!')
					reply(ind.wait())
				...
				case 'mining':
                      ...
                      if (!isEventon) return reply(`Desculpe ${pushname} os eventos não foram ativados pelo Haruki`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`Iae Haruki, toma ae essa escola ${one}Xp `)
                      }else{
                      const mining = Math.ceil(Math.random() * 10000)
                      addLevelingXp(sender, mining)
                      await reply(`*parabéns* ${pushname} você ganha isso *${mining}Xp*`)
                      }
                    await limitAdd(sender)
					break
				case 'grup':
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`ihuul @${mentioned[0].split('@')[0]} perdeu ADM, otário kkkkkkk`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Parabéns🥳 @${mentioned[0].split('@')[0]} *você se mostrou útil para ser ADM* não faça nada de errado para não perder este cargo`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`ihuuul @${mentioned[0].split('@')[0]} óh o ban kkkkkkk `, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `𝗟𝗶𝘀𝘁a de ADM daqui *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('*TÁ ATIVADO, KARALHO* !!!')
						welkom.push(from)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ ação de boas-vindas e saída está ativada')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨C𝗘𝗦SO ❭ ação de boas-vindas e saída está desativada')
					} else {
						reply(ind.satukos())
					}
					break 
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSO ❭ NSFW está ativado')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗦𝗨CESSO ❭ NSFW está desativado')
					} else {
						reply(ind.satukos())
					}
				break
				...
				case 'clone':
			  	...
			  	case 'event':
					...
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*TÁ ATIVADO, KARALHO* !!!')
						event.push(from)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨CES𝗦O ❭ os eventos estão ativados neste grupo️*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨CESSO ❭ os eventos estão desativados neste grupo*')
					} else {
						reply(ind.satukos())
					}
					break 
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ 𝗦𝗨CESSO ❭ ANTILINK ATIVADO*')
					} else if (Number(args[0]) === 0) {
						antilink.splice(from, 1)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('*❬ 𝗦𝗨CESSO ❭ ANTILINK DESATIVADO*')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'setppbot':
					...
					reply('Obrigado pelo novo perfil, Haruki')
					break 
				case 'setpp': 
                        if (!isGroup) return reply(ind.groupo())
                       if (!isGroupAdmins) return reply(ind.admin())
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
                    prefix = args[0]
                    reply(`Alterar prefixo para ${prefix} com sucesso!`)
					break 
				case 'setmemlimit':
					if (args.length < 1) return
					if (!isOwner) return reply(ind.ownerb())
